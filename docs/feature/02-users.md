...
API สำหรับจัดการผู้ใช้ระบบ — เฉพาะ **SuperAdmin** (permission `users:read`). สอดคล้อง ENTITY_SPEC (User, shop_id scope). สมาชิกร้านใช้ Shops API: GET/POST `/api/shops/me`, `/api/shops/me/members` (ดู docs/feature/04-shops.md).
...
|--------|------|------|------------|
| GET | `/api/users` | Required (Bearer JWT) | `users:read` (Admin/SuperAdmin) |
...
- [ ] GET /api/users ด้วย token ที่ role ไม่มี users:read → 403
- [ ] GET /api/users ด้วย token Admin/SuperAdmin → 200 และ body มี key `users` (array)
